...
class HauptAktivitaet : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val workspaceDir = File(filesDir, "werkstatt")
        workspaceDir.mkdirs()
        File(workspaceDir, "src").mkdirs()
        File(workspaceDir, "logs").mkdirs()
        File(workspaceDir, "appbuilder").mkdirs()
        File(workspaceDir, "build").mkdirs()
        val workspace = workspaceDir.absolutePath
            ...
            AgentDashboard(
                ...
                onCheckOllama = { url, model ->
                    if (NativeBruecke.geladen) {
                        ...
                        val localClient = de.xrdoge.agent.laufzeit.LocalOllamaClient(
                            baseUrl = url,
                            model = model,
                            bridge = runtime.socketBridge,
                            logStream = runtime.logStream
                        )
                        localClient.reachable()
                    }
                },
                onRunAgent = { dir: String, task: String, ollamaUrl: String, model: String, iterations: Int ->
